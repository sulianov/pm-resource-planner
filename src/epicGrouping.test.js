...
describe("normalizeStory", () => {
  it("extracts key and summary", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ summary: "My story" })));
    expect(story.key).toBe("BMO-1");
    expect(story.summary).toBe("My story");
  });

  it("extracts numeric SP", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ sp: 5 })));
    expect(story.sp).toBe(5);
  });

  it("converts string SP to number", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ sp: "2.5" })));
    expect(story.sp).toBe(2.5);
  });

  it("defaults SP to 0 when missing or non-numeric", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ sp: null })));
    expect(story.sp).toBe(0);
  });

  it("parses analysisDue into a Date object", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ analysisDue: "2026-05-15" })));
  ...
  it("sets analysisDue to null when not provided", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ analysisDue: null })));
    expect(story.analysisDue).toBeNull();
  });

  it("passes through currentDevDue and currentTestDue as raw strings", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ devDue: "2026-06-01", testDue: "2026-07-01" })));
    expect(story.currentDevDue).toBe("2026-06-01");
    expect(story.currentTestDue).toBe("2026-07-01");
  });

  it("extracts epicKey", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ epicKey: "BMO-999" })));
    expect(story.epicKey).toBe("BMO-999");
  });

  it("sets epicKey to null when missing", () => {
    const fields = storyFields();
    delete fields[F_EPIC_LINK];
    const story = normalizeStory(makeIssue("BMO-1", fields));
    expect(story.epicKey).toBeNull();
  });

  it("extracts status name", () => {
    const story = normalizeStory(makeIssue("BMO-1", storyFields({ status: "Done" })));
    expect(story.status).toBe("Done");
  });

  it("handles completely empty fields gracefully", () => {
    const story = normalizeStory({ key: "BMO-X", fields: {} });
    expect(story.key).toBe("BMO-X");
    expect(story.sp).toBe(0);
  ...
  it("handles missing fields property", () => {
    const story = normalizeStory({ key: "BMO-Y" });
    expect(story.sp).toBe(0);
    expect(story.epicKey).toBeNull();
  });
});
...
describe("groupStoriesByEpic", () => {
  ...
  it("groups stories by epic key", () => {
    const issues = [
      ...
      makeIssue("BMO-3", storyFields({ epicKey: "BMO-200", sp: 1 })),
    ];
    const { epics, storyMap } = groupStoriesByEpic(issues);
    expect(epics).toHaveLength(2);
    expect(storyMap["BMO-100"]).toHaveLength(2);
    expect(storyMap["BMO-200"]).toHaveLength(1);
  });

  it("separates orphan stories (no epic link)", () => {
    const issues = [
      makeIssue("BMO-1", storyFields({ epicKey: "BMO-100" })),
      makeIssue("BMO-2", { ...storyFields(), [F_EPIC_LINK]: null }),
    ];
    const { epics, orphanStories } = groupStoriesByEpic(issues);
    expect(epics).toHaveLength(1);
    expect(orphanStories).toHaveLength(1);
    expect(orphanStories[0].key).toBe("BMO-2");
  });
  ...
  it("sets epic analysisDue to the latest story analysisDue", () => {
    const issues = [
      makeIssue("BMO-1", storyFields({ epicKey: "BMO-100", analysisDue: "2026-05-01" })),
      makeIssue("BMO-2", storyFields({ epicKey: "BMO-100", analysisDue: "2026-06-15" })),
      makeIssue("BMO-3", storyFields({ epicKey: "BMO-100", analysisDue: "2026-04-10" })),
    ];
    const { epics } = groupStoriesByEpic(issues);
  ...
  it("sets epic analysisDue to empty string when no story has one", () => {
    const issues = [
      makeIssue("BMO-1", storyFields({ epicKey: "BMO-100", analysisDue: null })),
    ];
    const { epics } = groupStoriesByEpic(issues);
  ...
  it("sets epic name and id from the epicKey", () => {
    const issues = [makeIssue("BMO-1", storyFields({ epicKey: "BMO-100" }))];
    const { epics } = groupStoriesByEpic(issues);
    expect(epics[0].name).toBe("BMO-100");
    expect(epics[0].epicKey).toBe("BMO-100");
    expect(epics[0].id).toBe("BMO-100__import");
  });
  ...
  it("storyMap contains Story objects, not raw issues", () => {
    const issues = [makeIssue("BMO-1", storyFields({ epicKey: "BMO-100", sp: 5 }))];
    const { storyMap } = groupStoriesByEpic(issues);
    // Story objects have .sp as a number (normalized), not a raw field value
    expect(typeof storyMap["BMO-100"][0].sp).toBe("number");
    expect(storyMap["BMO-100"][0].sp).toBe(5);
  });
