...
pm-resource-planner does all of that in the browser, reactively, with the dates round-tripping through the Jira REST API.
 
The feature that changed how planning conversations actually go is **reverse planning**: set the target date, and the Sprint Demand tab shows minimum required devs vs. actual staffing, sprint by sprint. Editing staffing re-runs the whole plan live — so a staffing negotiation with engineering leadership happens against recalculating dates in the same meeting, not in a follow-up email three days later.
 
## Design constraints
 
Built for use inside a regulated financial environment, which drove the architecture more than any technical preference:
 
- **Local-only by design.** No cloud deployment, no external services, no data leaving the machine. The Python proxy exists solely to handle Jira CORS locally; the bearer token is never persisted.
- **No UI library, no backend, no database.** Session state lives in `localStorage`. Every dependency is a liability in an environment where tooling gets security-reviewed — the dependency list is deliberately short.
- **Scope cuts were deliberate:** no multi-user support, no auth layer, no cloud sync. Each was considered and dropped because it added surface area without changing the answer to the planning questions above.
---
...
Editing *Staffed* in any sprint immediately re-runs the plan. Every Planned Dev Due date and Gantt block updates reactively across all tabs, so you can negotiate a staffing change with engineering leadership and watch the impact land in real time.
...
## Why 118 tests for an internal tool
 
The planning engine makes commitments — dates that get written back to Jira and quoted to stakeholders. A planner that's occasionally wrong is worse than Excel, because Excel at least shows you the formula. The test suite covers the packing algorithm, business-day math (weekends + Canadian bank holidays), concurrency caps, and reverse planning — the parts where a silent bug becomes a missed release date.
